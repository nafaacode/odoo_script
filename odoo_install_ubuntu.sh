...
# Choose the Odoo version. For example: 16.0, 15.0, 14.0. When using 'master' the master version will be installed.
OE_VERSION="17.0"
# Set the domain name
DOMAIN_NAME="_"
...
if [[ "$version" == "16.04" || "$version" == "18.04" || "$version" == "20.04" || "$version" == "22.04" || "$version" == "24.04" ]]; then
    echo -e "Run script on  Ubuntu $version." 
else
    echo -e "\n--------"
    echo -e "ERROR Not tested version => exit"
    exit 1
fi
...
#--------------------------------------------------
echo -e "\n---- Update Server ----"
# universe package is for Ubuntu version > 16 
...
python3.11 -m venv $OE_USER-venv
...
EOF
  ...
  sudo mv ~/odoo /etc/nginx/sites-available/$DOMAIN_NAME
  sudo ln -s /etc/nginx/sites-available/$DOMAIN_NAME /etc/nginx/sites-enabled/$DOMAIN_NAME
  sudo rm /etc/nginx/sites-enabled/default
  sudo service nginx reload
  sudo su root -c "printf 'proxy_mode = True\n' >> /etc/${OE_CONFIG}.conf"
  echo "Done! The Nginx server is up and running. Configuration can be found at /etc/nginx/sites-available/$DOMAIN_NAME"
else
...
if [ $INSTALL_NGINX = "True" ] && [ $ENABLE_SSL = "True" ] && [ $ADMIN_EMAIL != "odoo@example.com" ]  && [ $DOMAIN_NAME != "_" ];then
...
if [ $INSTALL_NGINX = "True" ]; then
  echo "Nginx configuration file: /etc/nginx/sites-available/$DOMAIN_NAME"
fi
echo "-----------------------------------------------------------"
